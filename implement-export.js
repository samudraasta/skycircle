...
// 1. Get the base64 from bg_base64.txt
let bgBase64 = '';
try {
    bgBase64 = fs.readFileSync('bg_base64.txt', 'utf8').trim();
} catch (e) {
    console.log("No bg_base64.txt found, falling back to regex extraction");
    const match = html.match(/const base64BgBoy = "(data:image\/jpeg;base64,[^"]+)";/);
    if (match) bgBase64 = match[1];
}

// 2. Generate the exportClassProfiles function
const exportScript = `
        ...
        htmlContent += \`
            <script>
                window.onload = () => {
                    setTimeout(() => {
                        window.print();
                    }, 1000); // wait for images to load before printing
                };
            </script>
        </body>
        </html>
...
// Append function to the script
html = html.replace('</script>', exportScript + '\n</script>');
fs.writeFileSync('mentor/buku-saku.html', html);
console.log('Export function injected successfully');
